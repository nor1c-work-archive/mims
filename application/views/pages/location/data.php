...
<script>
    $(document).ready(function() {
        table = $('#example').DataTable({
            ...
            "serverSide": true,
            "ajax": {
                "url" : "<?=site_url(uriSegment(1).'/data?locType='.strtoupper(uriSegment(3)))?>",
                'method': 'POST',
                'data': function(d) {
                ...
                foreach ($columns as $colKey => $colValue) { ?>
                    {
                        "render": function(data, type, row) {
                            <?php if ($colKey == 'locType') { ?>
                                var kode = '<?=uriSegment(3)?>';
                                return '<a href="#" onClick="detailButton('+row[0]+')"><?=env('L_'.strtoupper(uriSegment(3)))?>-'+row[0]+'</a>';
                            <?php } else { ?>
