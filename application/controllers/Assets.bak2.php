...
class Assets extends INS_Controller {
    ...
    public function __construct() {
        parent::__construct();
        $this->assetType            = uriSegment(3);
        
        $this->selectedColumns      =   array('idAsset' => 'ID', 'assetParent' => 'KODE SISTEM', 'assetName' => 'NAMA ASET');
        if ($this->assetType != 'MIC') {
            $this->selectedColumns      = array_merge($this->selectedColumns, array('propAdmin.idLocation' => 'LOKASI', 'propAdmin.condition' => 'KONDISI', 'propAdmin.status' => 'STATUS'));
        }
        $this->selectedColumns      = array_merge($this->selectedColumns, array('propAdmin.priceBuy' => 'HARGA BELI', 'propAdmin.procureDate' => 'TANGGAL PEROLEHAN'));
        if (uriSegment(3) == 'MIS') {
            $this->selectedColumns      = array_merge($this->selectedColumns, array('propInstrument.insCategory' => 'KATEGORI'));
        }
        $this->width                = uriSegment(3) == 'MIS' ? array(10, 10, 30, 30, 10, 10, 10, 15, 20) : array(10, 10, 30, 30, 10, 10, 10, 15);
        $this->searchableColumns    = array('idAsset' => 'KODE SISTEM', 'assetName' => 'NAMA ASET', 'propAdmin.priceBuy' => 'HARGA BELI', 'propAdmin.idLocation' => 'LOKASI');
        $this->totalUnusedColumn    = uriSegment(3) == 'MIS' ? 3 : 2;
    ...
    public function data() {
        ...
        if (isset($filters) && !empty($filters)) {
            if (isset($filters['searchKeyword1'])) {
                ...
                if ($filters['searchInsCategory'] != '') {
                    $parameters['directFilters']['LIKEAND'][] = array(
                        "column" => 'propInstrument.insCategory',
        ...
        if (inputGet('catCode') && inputGet('catCode') == 'MIS') {
            $this->selectedColumns      = array_merge($this->selectedColumns, array('propInstrument.insCategory' => 'KATEGORI'));
        }
            ...
            foreach ($data['asset'] as $assetKey => $assetValue) {
                ...
                for ($i=1; $i <= $qty; $i++) { // kalau langsung input piece
                    ...
                    $set[] = array(
                        ...
                        'assetParent' => isset($assetValue['assetParent']) ? zeroDefault($assetValue['assetParent']) : 0,
                        ...
                        'propAdmin' => array(
                            ...
                            'yearProcurement' => isset($assetValue['yearProcurement']) ? emptyDefault($assetValue['yearProcurement']) : '',
    ...
    public function importPreview() {
        ...
        if (isset($_FILES['file'])) {
            ...
            $tableHeader = array('KODE SISTEM', 'KODE KATALOG', 'NAMA ALIAS ASET', 'KONDISI', 'STATUS', 'LEVEL RESIKO', 'KEPEMILIKAN', 'LOKASI', 'HARGA BELI', 'TANGGAL PEROLEHAN', 'SUPPLIER', 'KETERANGAN');
    ...
    public function export() {
        ...
        $sheet_details = array(
                            0 => array(
                                'sheet_title'   => 'data',
                                'sheet_heading' => array('KODE SISTEM', 'NAMA ASSET', 'KODE KATALOG', 'HARGA BELI', 'TANGGAL PEROLEHAN'),
    ...
    function rekap() {
        ...
        $html = '<style>
                 ...
                 <h2>DAFTAR ASSET INSTRUMENT ' . (uriSegment(3) == 'MIS' ? 'SET' : '') . '</h2>
